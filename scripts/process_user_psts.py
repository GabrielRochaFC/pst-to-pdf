...
CONVERTER_SCRIPT = SCRIPT_DIR / "convert_eml_to_pdf.py"
...
def run_extraction(job: PstJob, force_extract: bool, dry_run: bool) -> tuple[bool, float]:
    if has_eml_files(job.eml_dir) and not force_extract:
        print(f"[{job.slug}] extraction skipped: existing EML files found")
        return False, 0.0

    command = ["readpst", "-e", "-D", "-o", str(job.eml_dir), str(job.pst_path)]
    print(f"[{job.slug}] extracting PST to {job.eml_dir}")
    if dry_run:
        print(f"[{job.slug}] dry-run extract command: {' '.join(command)}")
        return False, 0.0

    job.eml_dir.mkdir(parents=True, exist_ok=True)
    job.extract_log.parent.mkdir(parents=True, exist_ok=True)
    started_at = time.monotonic()
    with job.extract_log.open("w", encoding="utf-8") as logfile:
        logfile.write(f"Source PST: {job.pst_path}\nOutput EML dir: {job.eml_dir}\n")
        result = subprocess.run(command, stdout=logfile, stderr=subprocess.STDOUT, check=False)
    elapsed = time.monotonic() - started_at
    if result.returncode != 0:
        raise RuntimeError(f"readpst failed for {job.pst_path}; see {job.extract_log}")
    print(f"[{job.slug}] extraction finished elapsed={format_elapsed(elapsed)} log={job.extract_log}")
    return True, elapsed
...
def main() -> int:
    ...
    if not shutil.which("readpst") and not args.validate_only:
